...
//        Resource resource = resourceLoader.getResource(g.getMapPath());
    ...
    public void BuildGraph(String graphPath, String mapPath, String name, Integer floor) {
        ArrayList<Graph> graphs = (ArrayList<Graph>) graphRepository.findByName(name);
        Graph g = null;
        if (graphs.size() > 0) {
...
//		Navigate(g, Nodes.get(a), Nodes.get(b), "src/main/resources/static/result");
    ...
    public ArrayList<BufferedImage> Navigate(String building1, String building2, Node start, Node des, String resultPath) {
        ...
        if(building1.equals(building2)) {
            ...
            try {
                d1 = new Draw(Route1, (BufferedImage) FileUtils.openResImage(FileUtils.getMapPath(FileUtils.getFileName(building1, start.floor))));
                d1.drawRoute();
                d2 = new Draw(Route2, (BufferedImage) FileUtils.openResImage(FileUtils.getMapPath(FileUtils.getFileName(building1, des.floor))));
                d2.drawRoute();
            } catch (IOException e) {
                e.printStackTrace();
            }
